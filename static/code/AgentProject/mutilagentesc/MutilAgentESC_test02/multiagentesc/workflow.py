"""
Main workflow for MultiAgentESC reproduction on MASFactory.
Aligned with source: main.py main loop + multiagent.py inner logic.
"""

import re
# Apply patches before other MASFactory imports so Agent is already monkey-patched.
from .patch_masfactory import PlainTextFormatter  # noqa: F401
...
def _create_graph_with_agent(model, name, prompt_template, instructions,
    ...
    """Helper: create a reusable one-node graph."""
    kwargs = dict(
        cls=Agent,
        name="agent",
        model=model,
...
def _make_is_complex_wrapper(is_complex_graph):
    """Create is_complex wrapper. Source: multiagent.py is_complex()."""
...
def _make_judge_wrapper(judge_graph):
    """Create judge wrapper. Source: multiagent.py judge()."""
...
def _make_self_reflection_wrapper(reflection_graph):
    """Create self-reflection wrapper. Source: multiagent.py self_reflection()."""
    def self_reflection(context, pred_strategy, response):
        ...
        try:
            ...
            # Lenient fallback: try "Response: <text>" without brackets
        ...
        # Strip trailing "Reasoning: ..." from response
...
def create_workflow(model):
    """Build all graphs and return (graphs, utils) needed by process().

    Source: main.py + multiagent.py initialization.
    Model params from source: temperature=0.0, cache_seed=2024.
    max_tokens: 100 for is_complex/zero_shot/response_gen, 400 for all others.
    """
    # Source LLM params: temperature=0.0 for all agents
    settings_100 = {"max_tokens": 100}   # is_complex, zero_shot, response_with_strategy
    settings_400 = {"max_tokens": 400}   # emotion, cause, intention, judge, self_reflection
    ...
    # Source: response_with_strategy uses max_tokens=100
    ...
    # judge: input key "template_responses" must match {template_responses} placeholder
    ...
    # self-reflection: source uses max_tokens=400
    ...
    # Multi-agent: strategy discussion (max_tokens=400 per agent)
...
def process(message, attributes, model, graphs, utils):
    """Outer loop: iterate dialog, generate replies.
    Strictly aligned with source: main.py main loop (lines ~162-284).
    """
    ...
    while count < len(dialog):
        ...
        if count != 0 and dialog[count]["speaker"] == "supporter":
            # === Determine single vs consecutive supporters ===
            # Source: main.py lines 186-189 (single) vs 201-219 (consecutive)
            is_single = (
            ...
            # Build context BEFORE adding current turn (source: json2natural(history))
            ...
            # Source: main.py line 221 — "if count <= 5 or not is_complex(...)"
            if count <= 5 or not utils["is_complex"](context):
                ...
                # === Complex: emotion → cause → intention ===
                ...
                try:
                    ...
                    emotion = "Negative"
                emo_and_reason = emo_result  # Full response for subsequent prompts
                ...
                if len(pred_strategies) == 0:
                    ...
                    if len(pred_strategies) == 1:
                        # Single strategy: generate response directly
                        ...
                        # Multi-strategy: generate responses, debate, reflect, vote
                        responses_list = []
                        ...
                        # Clean strategies from vote — model may output non-standard names
                        ...
                        if len(strats) == 1 and strats[0] != "None":
                            ...
                            # Clean judge output — same reason as vote
                    ...
                    # === Self reflection (only when strategies > 0) ===
                    # Source: main.py line 271 — inside `else` of `if len(pred_strategy) == 0`
                    pred_str = save["pred_strategy"]
                    orig_resp = save.get("ori_response", "")

                    # Validate strategy — vote/judge may return non-standard names
                    if not orig_resp or orig_resp == "None":
                        # No valid response generated, fall back to zero_shot
                        out_zs, _ = graphs["zero_shot"].invoke({"context": context})
                        save["response"] = utils["parse_single_response"](out_zs.get("response", ""))
                        save["pred_strategy"] = "None"
                    else:
            ...
            # Non-supporter turn: just add to history
            history.append({
                "content": dialog[count]["content"].strip(),
                "role": "user" if dialog[count]["speaker"] == "seeker" else "assistant"
            })
