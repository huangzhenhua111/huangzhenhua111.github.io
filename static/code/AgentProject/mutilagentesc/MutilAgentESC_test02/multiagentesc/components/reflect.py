...
Aligned with source: multiagent.py reflect()

Source uses autogen GroupChat (admin + N agents, round-robin, max_round=N+1).
This uses serial agent calls with accumulated reflection history.
"""
...
def run_reflect(model: OpenAIModel, context, emo_and_reason, cau_and_reason, int_and_reason,
    ...
    """Run multi-agent reflection on debate results.

    Source config (from multiagent.py reflect):
    - admin prompt: reflect template with discussion_content = joined debate_history
    - agent system_message: includes "Your thought can be changed if you believe..."
    - temperature: 0.0, max_tokens: 400, cache_seed: 2024
    - max_round: len(responses) + 1

    Returns list of reflection opinion strings (one per agent).
    """
    discussion_content = "\n\n".join(debate_history)
...
Reasoning: [reasoning]"""
    ...
    for i in range(len(responses)):
        ...
        agent = RootGraph(f"reflect_turn_{i}")
        node = agent.create_node(Agent, f"reflector_{i}",
            model=model,
            instructions=(
        ...
        agent.edge_from_entry(node, {"reflect_prompt": ""})
        agent.edge_to_exit(node, {"reflection": ""})
        agent.build()
        out, _ = agent.invoke({"reflect_prompt": current_prompt})
