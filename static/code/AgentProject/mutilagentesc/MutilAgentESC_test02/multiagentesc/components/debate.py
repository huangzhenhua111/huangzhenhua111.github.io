"""
Debate component: multi-agent group discussion to select best response.
Aligned with source: multiagent.py debate()

Source uses autogen GroupChat (admin + N agents, round-robin, max_round=N+1).
This implementation uses serial agent calls with accumulated debate history,
which is functionally equivalent since each agent speaks exactly once.
"""
...
def run_debate(model: OpenAIModel, context, emo_and_reason, cau_and_reason, int_and_reason, responses):
    """Run multi-agent debate to select best response.

    Source config (from multiagent.py debate):
    - admin prompt: debate template with context, emo, cause, intention, responses_template
    - agent system_message: "You are a psychologist who is good at listening..."
      + inclined to support responses[i]
    - temperature: 0.0, max_tokens: 400, cache_seed: 2024
    - max_round: len(responses) + 1

    Returns list of debate opinion strings (one per agent, excluding admin's initial message).
    """
    responses_template = "\n\n".join(responses)
...
Reasoning: [reasoning]"""
    ...
    for i in range(len(responses)):
        # Build prompt: admin prompt + accumulated debate history
        if debate_history:
            history_text = "\n\n".join(debate_history)
        ...
        # Source: agent system_message includes inclined response
        agent = Agent(
            name=f"debater_{i}",
            model=model,
            instructions=(
